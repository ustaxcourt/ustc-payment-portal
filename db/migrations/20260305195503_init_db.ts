...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable("transactions", (t) => {
    ...
    t.string("paygov_tracking_id", 21)
      .nullable()
      .comment("Pay.gov Tracking ID (optional)");
    t.string("transaction_reference_id", 36)
      .notNullable()
    ...
    t.timestamp("last_updated_at", { useTz: true })
      .notNullable()
      .defaultTo(knex.fn.now());
    t.unique(
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema.raw(
    "DROP TRIGGER IF EXISTS trg_transactions_last_updated_at ON transactions",
  );
