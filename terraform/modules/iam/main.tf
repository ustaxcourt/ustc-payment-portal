...
# Route53 and ACM permissions are only needed in environments with a custom domain
# (dev, stg, prod). PR ephemeral environments do not have a hosted zone.
resource "aws_iam_role_policy" "github_actions_custom_domain" {
  count = var.create_deployer_role && var.route53_zone_id != "" ? 1 : 0
  name  = "${local.project_name}-${local.environment}-ci-deployer-custom-domain"
  role  = aws_iam_role.github_actions_deployer[0].id
