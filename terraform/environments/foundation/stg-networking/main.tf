...
# =============================================================================
# Playwright Staging e2e test (run-lambda worker + API SigV4 signing)
# =============================================================================
# Register this role ARN in client-permissions before load testing.

resource "aws_iam_role" "github_actions_playwright_e2e" {
  name = "${local.name_prefix}-github-playwright-e2e-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            "token.actions.githubusercontent.com:sub" = "repo:ustaxcourt/ustc-payment-portal:*"
          }
        }
      }
    ]
...
resource "aws_iam_role_policy_attachment" "playwright_e2e_test_basic_execution" {
  role       = aws_iam_role.github_actions_playwright_e2e.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

resource "aws_iam_role_policy" "playwright_e2e_test" {
  name = "playwright-e2e-test"
  role = aws_iam_role.github_actions_playwright_e2e.id
