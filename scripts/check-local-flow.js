...
function parseToken(initResponseBody) {
	if (
		typeof initResponseBody.token === "string" &&
		initResponseBody.token.length > 0
	) {
...
function selectScenario(fee) {
	...
	if (!match) {
		throw new Error(
			`Unknown FEE: ${fee}. Supported: ${Object.values(TEST_DATA)
				.map((s) => s.fee)
				.join(", ")}`,
		);
	}
...
async function main() {
	...
	const paymentBase =
		process.env.PAYMENT_URL || `http://localhost:${paymentPort}/pay`;
	const scenario = selectScenario(
		process.env.FEE || TEST_DATA.petitionFiling.fee,
	);
	...
	const looksLikeHtmlDoc =
		lowerHtml.includes("<html") && lowerHtml.includes("</html>");
	const isMockPayPage =
