...
const mockSend = (SecretsManagerClient as jest.Mock).mock.results[0]?.value
	?.send;

describe("getSecretString", () => {
	it("throws an error if secretId is missing", async () => {
		await expect(getSecretString("")).rejects.toThrow(
			"getSecretString: secretId is required",
		);
	});
	...
	it("handles AWS SDK errors gracefully", async () => {
		...
		await expect(getSecretString("restricted-secret")).rejects.toThrow(
