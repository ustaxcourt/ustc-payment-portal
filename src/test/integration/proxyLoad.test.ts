...
// AC #2 proof: drives concurrent /init traffic so we can watch the RDS Proxy pool/multiplex
// connections in CloudWatch (DatabaseConnections should stay low; SessionPinned ~0).
...
const runOrSkip = process.env.RUN_LOAD_TEST === "true" ? describe : describe.skip;
...
runOrSkip("RDS Proxy load test (AC #2)", () => {
  it(`sustains ${CONCURRENCY} concurrent /init for ${DURATION_MS}ms`, async () => {
    ...
    const worker = async (): Promise<void> => {
      while (Date.now() < deadline) {
        ...
        try {
          const res = await signedFetch(`${BASE_URL}/init`, { method: "POST", headers, body });
          total += 1;
    ...
    const pct = (p: number): number => okLatencies[Math.floor(okLatencies.length * p)] ?? 0;
    const breakdown = [...statusCounts.entries()].map(([k, v]) => `${k}=${v}`).join(" ");
    console.log(
      `\nLoad summary: total=${total} ok=${ok} concurrency=${CONCURRENCY} duration=${DURATION_MS}ms\n` +
        `  status: ${breakdown}\n` +
        `  ok latency: p50=${pct(0.5)}ms p95=${pct(0.95)}ms p99=${pct(0.99)}ms\n`,
    );
