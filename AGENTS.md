...
- Fees are hardcoded in `src/config/fees.ts` (staticFees), not a DB table. A stable fee key can have multiple versioned FeeVersions; getActiveFee(fee, date) resolves to the version with the latest activationDate <= date, throwing an error if the key is unknown or no version has activated yet. Callers pass the transaction's createdAt (not "now") so historical rows stay pinned to the version in effect at creation time. See [docs/architecture/API-Documentation/supported_court_fees_and_client_auth.md](vscode-webview://151648ku37mfu7jarf57t13on0rbvvm1ffq09dpmsj1t60t25f78/docs/architecture/API-Documentation/supported_court_fees_and_client_auth.md) for the full request/validation flow.
- Changes to `src/config/fees.ts` affect real money charged to the public. If a change is needed, suggest it to the developer — do not make it yourself. Only the developer is permitted to update or add fees.
...
- **Comments**: a comment must surface something the code cannot — an invariant, a trade-off, why an alternative was rejected, or what breaks if the line changes. Apply this test: **if the comment would still make sense with the code deleted, it is narrating control flow — delete the comment instead.** Keep what survives to one or two lines. Operational guidance belongs where it is read (a CloudWatch alarm description, a runbook), not duplicated inline. This governs explanatory comments, not API documentation on exported members — e.g. `TransactionModel.queryLog`'s doc comment describing its ordering contract is worth keeping.
  - Good: [`src/health/cancelSweepMetric.test.ts`](src/health/cancelSweepMetric.test.ts) — *"A quiet sweep must still emit, or the alarm cannot tell 'nothing expired' from 'the sweeper stopped running'"*. States a consequence of removing the call; meaningless without it.
  - Good: [`db/migrations/20260914000000_last_updated_at_respects_explicit_value.ts`](db/migrations/20260914000000_last_updated_at_respects_explicit_value.ts) — records that a `BEFORE UPDATE` trigger cannot distinguish a self-assignment from an absent column, so nobody reimplements the approach that does not work.
  - Bad: restating an adjacent assignment (`// cancelled workflow state, failed business outcome` above `transactionStatus: "cancelled"`).
  - Bad: explaining how to interpret a metric in the emitter, when the alarm description already says it.
