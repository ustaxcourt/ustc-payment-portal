...
- **Omitting a `feeKey` from `allowedFeeKeys`** — blocks that app from charging the fee. Requests with an unauthorized fee key return `403 Forbidden` with `"Client not authorized for fee"`.
