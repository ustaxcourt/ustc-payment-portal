# PAY-302: Logging Library Comparison — Pino vs Winston vs AWS Lambda Power Tools

## Purpose

This document compares three logging solutions researched for PAY-302 to support the team's selection decision. Each option has a dedicated implementation plan in this folder. This document focuses on trade-offs relevant to the Payment Portal's context: AWS Lambda, CloudWatch, TypeScript, structured JSON, and developer experience.

---

## At a Glance

|                                 | Pino                                           | Winston                                         | AWS Lambda Power Tools                           |
| ------------------------------- | ---------------------------------------------- | ----------------------------------------------- | ------------------------------------------------ |
| **GitHub stars**                | ~18k                                           | ~23k                                            | ~4k                                              |
| **Maintained by**               | Community                                      | Community                                       | AWS                                              |
| **Last active**                 | Active                                         | Active                                          | Active                                           |
| **JSON output by default**      | Yes                                            | Requires config                                 | Yes                                              |
| **Log levels**                  | trace/debug/info/warn/error/fatal/silent       | error/warn/info/http/verbose/debug/silly        | debug/info/warn/error/fatal/silent               |
| **Level values in JSON**        | Numbers (configurable to strings)              | Strings (configurable)                          | Numbers only                                     |
| **Message key**                 | `msg`                                          | `message`                                       | `message`                                        |
| **Timestamp key**               | `time`                                         | `timestamp`                                     | `timestamp`                                      |
| **Child logger**                | `logger.child()`                               | `logger.child()`                                | `logger.createChild()`                           |
| **Built-in redaction**          | Yes (`redact` option, path-based)              | No (manual)                                     | Yes (`maskJsonValues()`)                         |
| **Pretty local output**         | `pino-pretty` (transport, dev dependency)      | Built-in colorize/printf formatters             | None (always JSON)                               |
| **TypeScript support**          | First-class (`pino` ships types)               | Good (`@types/winston` not needed; ships types) | First-class (TypeScript-first library)           |
| **Performance**                 | Fastest (offloads formatting to worker thread) | Moderate                                        | Very fast (stderr-only, Lambda-optimized)        |
| **Lambda context auto-inject**  | Manual                                         | Manual                                          | Automatic                                        |
| **Extra fields always present** | `pid`, `hostname` (suppressible)               | None beyond what you configure                  | Lambda context fields only (function name, etc.) |
...
- **`http` and `verbose` levels** are not standard across logging libraries. They have no equivalent in Pino or AWS Lambda Power Tools, which can cause confusion in a mixed codebase or when writing CloudWatch filter rules.
...
- **Zero external dependencies for logging** — The logger works out of the box without requiring `pino-pretty` or other external tools.
...
- **Numeric log levels only** — Power Tools uses numeric level values in JSON output (`20` for `INFO`). CloudWatch filter expressions must use `level >= 20` rather than string matches.
...
| Environment | Pino default | Winston default | AWS Lambda Power Tools default |
| ----------- | ------------ | --------------- | ------------------------------ |
...
All three options support `LOG_LEVEL` override in any environment using the same resolution logic.

### Local Developer Experience

|                         | Pino                                 | Winston                        | AWS Lambda Power Tools                   |
| ----------------------- | ------------------------------------ | ------------------------------ | ---------------------------------------- |
| Pretty output mechanism | `pino-pretty` transport (dev dep)    | Built-in `colorize` + `printf` | Pipe through `jq`                        |
| Setup required          | `npm install --save-dev pino-pretty` | None                           | None (but `jq` is recommended)           |
| Output on by default    | No (must configure transport)        | Yes                            | Yes (raw JSON to stderr)                 |
| Color support           | Yes (via pino-pretty)                | Yes (via colorize format)      | No (pipe to jq for color)                |
| Field filtering         | No built-in (CLI tooling external)   | No built-in                    | Yes (`jq 'select(.feeId == "FEE-001")'`) |
...
| Timestamp key               | `time`                           | `timestamp`          | `timestamp`                                               |
| Extra always-present fields | `pid`, `hostname` (suppressible) | None                 | Lambda context only (`function_name`, `request_id`, etc.) |
| Built-in redaction          | Yes                              | No                   | Yes                                                       |
| Automatic Lambda context    | No (manual)                      | No (manual)          | Yes (automatic from Lambda runtime)                       |
...
| Relative throughput | Fastest               | Moderate           | Very fast              |
| Cold start impact   | Minimal               | Low-moderate       | Minimal                |
