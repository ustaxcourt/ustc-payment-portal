...
- Each attempt has its own status (`Received`, `Initiated`, `Pending`, `Completed`).
- Each attempt keeps track of the payment status as reported by Pay.gov's `transaction_status` (`Pending`, `Success`, `Failed`)
