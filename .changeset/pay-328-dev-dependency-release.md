...
- **CLI entrypoint**: `npx ustc-payment-portal start` brings up the full local stack (Payment Portal API + Pay.gov Test Server + Postgres) with zero configuration required.
...
- **Pre-compiled migrations and seeds** ship in `dist/db/` — consumers do not need `ts-node` or TypeScript tooling to run the portal.
